...
async function testIntegration() {
    ...
    try {
        ...
        const invalidProjectRes = await fetch(`${API_URL}/projects`, {
            method: 'POST',
            headers: {
        ...
        const invalidProjectData = await invalidProjectRes.json() as any;
        ...
        const deleteRes = await fetch(`${API_URL}/projects/${projectId}`, {
            method: 'DELETE',
        ...
        const projectStillVisible = getProjectsData.data.some((p: any) => p.id === projectId);
        ...
        // 8. Delete Event
        console.log('\n8. Deleting Event...');
        const deleteEventRes = await fetch(`${API_URL}/gallery/events/${eventId}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${token}` }
        });

        const deleteEventData = await deleteEventRes.json() as any;
